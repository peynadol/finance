...
import PotsCard from "../components/pots/pots-card"; // ✅ switched
...
const PotsPage = () => {
  ...
  const potTotals: Record<string, number> = useMemo(() => {
    return potsData.reduce((acc, pot) => {
      const total = transactions.reduce((sum, transaction) => {
        return transaction.category === pot.name
          ? sum + transaction.amount
          : sum;
      }, 0);
      return { ...acc, [pot.id]: total };
    }, {});
  }, [potsData, transactions]);
